...
这会使标题栏横向区域与左侧侧边栏形成连续的 L 形视觉结构。36 px 高的拖拽区以 WebUI 侧边栏的实时右边缘为左界、以平台原生窗口按钮为右界；通过单个 `ResizeObserver` 跟随侧边栏展开、收起和过渡，不新增侧边栏管理按钮。原生窗口按钮位于右侧的平台还应通过稳定的 `conversation.session.header.utilities` slot 为 WebUI header 保留同宽安全区，并将该 utilities 交互层置于拖拽区之上；不得依赖 Session log 文案或 CSS Modules 生成类名。
