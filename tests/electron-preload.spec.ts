...
describe('Electron preload title bar', () => {
  ...
  it('waits for the WebUI layout and preserves the native left inset', async () => {
    ...
    expect(harness.style.textContent).toContain('conversation.session.header.utilities')
    expect(harness.style.textContent).toContain('-webkit-app-region:no-drag')
    expect(harness.style.textContent).not.toContain('padding-right:148px')
  })
})
