...
describe('Electron preload title bar', () => {
  it('starts the drag region after the live sidebar edge', async () => {
    ...
    expect(harness.style.textContent).toContain('left:50%;right:138px;height:36px')
    expect(harness.style.textContent).toContain("header:has([data-slot='conversation.session.header.utilities']){padding-right:138px!important}")
    expect(harness.style.textContent).toContain("header :has(>[data-slot='conversation.session.header.utilities']){position:relative;z-index:2147483647}")
