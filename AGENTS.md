...
This repository contains the `@anestis/dsh-desktop` dsh plugin. Keep architecture notes in `docs/`; runtime code is in `src/`, tests in `tests/`, and the bundle composer in `scripts/`. `lib/` is generated by the build and must not be edited by hand. The Electron preload is the deliberately isolated CommonJS exception at `src/electron-preload.cjs`.
